...
def dashboard(request):
    ...
    vaccinated_obj = CustomUser.objects.filter(vaccination_status=True).count()
    total_registered = CustomUser.objects.all().count()
    return render(request, 'dashboard.html', {"data": vaccination_obj.data, "data1": vaccinated_obj, "data2": total_registered})
...
def updaterecord(request, id):
  ...
  try:
    ...
    checks = 'off'
  is_slot_done = True if checks == "on" else False
  ...
  member.is_slote_done = is_slot_done
  member.save()
  return HttpResponseRedirect(redirect_to='/dashboard')